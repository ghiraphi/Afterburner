#проверка двух случайных регулярок,
# где 1- match, регулярки похожи, одна дублирует другую,
# 0 - не match, регулярки уникальны.
# -*- coding: utf-8 -*-
import re
    ...
    if word_from_line_key[0] != '*' and word_from_line_key[len(word_from_line_key) - 1] == '*':
        ...
        if word_from_line_exc[0] != '*' and word_from_line_exc[len(word_from_line_exc) - 1] != '*' and re.search('\w+\*\w+', word_from_line_exc) == None and (
            ...
            for index_letter_key, letter_key in list(enumerate(word_from_line_key[:len(word_from_line_key) - 1])):
                for index_letter_exc, letter_exc in list(enumerate(word_from_line_exc)):  # проверка межзвёзднной части на совпадение
                    if (fix_index_letter_key == 0 and fix_index_letter_exc == 0 and index_letter_key==0 and index_letter_exc==0) or (index_letter_exc == fix_index_letter_exc + 1 and index_letter_key == fix_index_letter_key + 1):
...
def add_dist(a):
    ...
    return a
# основная функция comparing
def comparing(dict1, dict2): # 1 - match, 2 - not match
    dict1=[repreg(dict1)]
    dict2=[repreg(dict2)]
        ...
        for orig_index_line2, orig_line2 in enumerate(dict2):
            for new_line1 in dict1[orig_index_line1]:
                for new_line2 in dict2[orig_index_line2]:
                    if compare_lines_new(new_line1, new_line2) == 1:
                        print('da')
                        return 1
                    else:
                        if orig_index_line1==len(dict1)-1 and orig_index_line2==len(dict2)-1:
                            temp = dict1
                            dict1 = dict2
                            dict2 = temp
                            for orig_index_line1, orig_line1 in enumerate(dict1):
                                    ...
                                    for new_line1 in dict1[orig_index_line1]:
                                        for new_line2 in dict2[orig_index_line2]:
                                            if compare_lines_new(new_line1, new_line2) == 1:
                                                ...
                                                if orig_index_line1 == len(dict1) - 1 and orig_index_line2 == len(
                                                        dict2) - 1:
                                                    print('not match')
                                                    return 0 #основной ##
