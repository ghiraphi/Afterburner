#программа для удаления дублей регулярок

def del_reg(enter_list):
    import re
    import time
    def file(link):
        original_list_key=[]
    ...
    def repreg(base_line):
        ...
        base_line = base_line.replace('[А-Я][а-я]', '.').replace('[A-Z][a-z]', '.').replace('[а-я]', '.').replace('[a-z]', '.').replace('[а-яa-z]', '.').replace('[a-zа-я]', '.').replace('[^а-я]', '£').replace('[1-9]', '£').replace('[0-9]', '£').replace('+', '*').replace('&#091', '©091').replace('&#093', '©093').replace('&#092', '©092').replace('&#047', '©047').replace('&#094', '©094').replace('&#036', '©036').replace('&#046', '©046').replace(
        ...
        if word_from_line_key[0] == '*' and word_from_line_key[len(word_from_line_key) - 1] == '*':
            ...
            if word_from_line_exc[0] == '*' and word_from_line_exc[
                ...
                if len(word_from_line_exc[1:len(word_from_line_exc) - 1]) == len(word_from_line_key[
                                                                                 1:len(word_from_line_key) - 1]):  # сравнить к и и  - начать итерацию, в зависимости от размера меж звёзд
                    counter_match = 0
                    ...
                    for index_letter_exc, letter_exc in reversed(list(enumerate(word_from_line_exc[
                                                                                1:len(word_from_line_exc) - 1]))):  # проверка межзвёзднной части на совпадение
            ...
            if word_from_line_exc[0] != '*' and word_from_line_exc[len(word_from_line_exc) - 1] == '*':  # искл - абв*
                if len(word_from_line_exc[:len(word_from_line_key) - 1]) == len(
                    ...
                    fix_index_letter_exc = 0
                    for index_letter_exc, letter_exc in reversed(list(enumerate(word_from_line_exc[
                                                                                :len(word_from_line_key) - 1]))):  # проверка межзвёзднной части на совпадение
                        for index_letter_key, letter_key in reversed(
                                list(enumerate(word_from_line_key[1:len(word_from_line_key) - 1]))):
                            if (fix_index_letter_key == 0 and fix_index_letter_exc == 0) or (
                                    index_letter_exc == fix_index_letter_exc - 1 and index_letter_key == fix_index_letter_key - 1):
                ...
                if len(word_from_line_exc[:len(word_from_line_key) - 1]) > len(word_from_line_key[
                    ...
                    for index_letter_key, letter_key in reversed(
                            list(enumerate(word_from_line_key[1:len(word_from_line_key) - 1]))):
                        for index_letter_exc, letter_exc in reversed(list(enumerate(word_from_line_exc[
                                                                                    :len(word_from_line_exc)]))):  # проверка межзвёзднной части на совпадение
                            if (fix_index_letter_key == 0 and fix_index_letter_exc == 0) or (
                                    index_letter_exc == fix_index_letter_exc - 1 and index_letter_key == fix_index_letter_key - 1):
                                if letter_key == letter_exc or letter_key == '.' or (
                                        letter_key == '£' and re.fullmatch('\d', letter_exc)) or (
        ...
        if word_from_line_key[0] != '*' and word_from_line_key[len(word_from_line_key) - 1] == '*':
            ...
            if word_from_line_exc[0] != '*' and word_from_line_exc[
                ...
                if len(word_from_line_exc[:len(word_from_line_exc) - 1]) > len(word_from_line_key[
                                                                               :len(word_from_line_key) - 1]):  # сравнить к и и  - начать итерацию, в зависимости от размера меж звёзд
                    counter_match = 0
                    fix_index_letter_key = 0
                    fix_index_letter_exc = 0
                    for index_letter_key, letter_key in reversed(
        ...
        if word_from_line_key[0] == '*' and word_from_line_key[len(word_from_line_key) - 1] != '*':
            ...
            if word_from_line_exc[0] != '*' and word_from_line_exc[
                ...
                if len(word_from_line_exc[:len(word_from_line_exc) - 1]) == len(word_from_line_key[1:]) and (
                    ...
                    fix_index_letter_exc = 0
                    for index_letter_exc, letter_exc in reversed(list(enumerate(word_from_line_exc[
                                                                                :len(word_from_line_exc) - 1]))):  # проверка межзвёзднной части на совпадение
                        for index_letter_key, letter_key in reversed(list(enumerate(word_from_line_key[1:]))):
                            if (fix_index_letter_key == 0 and fix_index_letter_exc == 0) or (
                                    index_letter_exc == fix_index_letter_exc - 1 and index_letter_key == fix_index_letter_key - 1):
                                if letter_key == letter_exc or letter_key == '.' or (
    ...
    def main_compare():
        if orig_index_line1 != len(dict1) - 1:
            count_orig_line2 = 0
            if len(dict1[orig_index_line1]) <= 100:
                for orig_index_line2, orig_line2 in enumerate(dict1):
                    if len(dict1[orig_index_line2]) <= 10 :
                        count_orig_line2 += 1
    ...
    # original_list=file('C:\\tmp\\exc_sui.txt') #получает исходный список регулярных выражений
    dict1=[]
    dict2=[]
    for original_line in enter_list:
    ...
    print('итерация по обраному списку-', original_list)
    ...
    for orig_index_line1, orig_line1 in enumerate(dict1):
        ...
        main_compare()
    original_list=[k for k in original_list if k != '¶']
    print(len(original_list), '- столько строк осталось2', original_list)

    print('запись в файл - конец')
    #file_for_record = open('rezult_compare_keys_exc.txt', 'w',  encoding="utf-8")
    # for newlinerec in original_list:
    #     file_for_record.write(str(newlinerec + '\n'))
    # file_for_record.close()
    return original_list #выводит изменённый с сокращениями новый список регулярок